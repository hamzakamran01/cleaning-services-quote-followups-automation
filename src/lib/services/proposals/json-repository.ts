...
export function getStoreSnapshotForFollowUp() {
  const store = getStore();
  return {
    company: store.company,
    followUpSequences: store.followUpSequences,
    trackingEvents: store.trackingEvents,
    followUpLogs: store.followUpLogs,
  };
}

export function appendFollowUpLog(data: {
  proposalId: string;
  sequenceId: string;
  sequenceStep: number;
  triggerEvent: string;
  ...
  return mutateStore(({ followUpLogs, proposals }) => {
    followUpLogs.push({
      id: crypto.randomUUID(),
    ...
    const idx = proposals.findIndex((p) => p.id === data.proposalId);
    if (idx !== -1) {
      proposals[idx].followUpCount += 1;
